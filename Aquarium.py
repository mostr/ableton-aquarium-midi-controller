...
class Aquarium(ControlSurface):
    __doc__ = " Script for Aquarium in APC emulation mode "

    _active_instances = []
    def _combine_active_instances():
        track_offset = 0
        scene_offset = 0
        for instance in Aquarium._active_instances:
    ...
    def _do_combine(self):
        if self not in Aquarium._active_instances:
            Aquarium._active_instances.append(self)
            Aquarium._combine_active_instances()


    def _do_uncombine(self):
        if ((self in Aquarium._active_instances) and Aquarium._active_instances.remove(self)):
    ...
    def _setup_session_control(self):
        is_momentary = True
        self._session = SpecialSessionComponent(REDBOX_TRACKS_NUM, REDBOX_SCENES_NUM)
        ...
        self._scene_launch_buttons = [self._note_map[SCENELAUNCH[index]] for index in range(REDBOX_SCENES_NUM) ]
        self._track_stop_buttons = [self._note_map[TRACKSTOP[index]] for index in range(REDBOX_TRACKS_NUM) ]
        ...
        self._session.selected_scene().set_duplicate_button(self._note_map[SELSCENEDUPLICATE])
        self._session.set_slot_launch_button(self._note_map[SELCLIPLAUNCH])
        for scene_index in range(REDBOX_SCENES_NUM):
            scene = self._session.scene(scene_index)
            scene.name = 'Scene_' + str(scene_index)
            button_row = []
            scene.set_launch_button(self._scene_launch_buttons[scene_index])
            scene.set_triggered_value(2)
            for track_index in range(REDBOX_TRACKS_NUM):
    ...
    def _setup_mixer_control(self):
        is_momentary = True
        self._mixer = SpecialMixerComponent(REDBOX_TRACKS_NUM)
        ...
        for track in range(REDBOX_TRACKS_NUM):
    ...
    def _setup_device_and_transport_control(self):
        ...
        for index in range(REDBOX_TRACKS_NUM):
